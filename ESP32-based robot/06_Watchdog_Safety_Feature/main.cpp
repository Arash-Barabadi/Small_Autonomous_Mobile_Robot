...
static constexpr uint32_t CMD_TIMEOUT_US = 250000; // 250000 microseconds == 250 milliseconds
...
// ================================= Motor drive ==================================
// here I define a function which controls the direction and speed of only ONE motor


void setMotor(int in1, int in2, int pwm_chan, int16_t speed_pwm) {   // pwm_chan = 0 → left motor gets PWM &  pwm_chan = 1 → right motor gets PWM
  // sets the PWM duty cycle for motor speed
  uint8_t duty = (uint8_t)constrain((int)abs(speed_pwm), 0, 255);

 //  digitalWrite(in1, HIGH); means  : Turn ON the control signal on pin in1 → send 3.3 volts to the motor driver so the motor spins in the chosen direction.
  if (speed_pwm > 0) { digitalWrite(in1, HIGH); digitalWrite(in2, LOW); } // Forward movement
  else if (speed_pwm < 0) { digitalWrite(in1, LOW); digitalWrite(in2, HIGH); } // Backward movement
  else { digitalWrite(in1, HIGH); digitalWrite(in2, HIGH); }                  // active brake


  ledcWrite(pwm_chan, duty); // when we call ledcWrite, The ESP32 looks at PWM channel 0(left side) or 1(right side) and sends a voltage signal (duty) to the pin attached to that channel
